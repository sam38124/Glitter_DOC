...
var ArticleContent=createReactClass({
    render:function (){
            return  <div id="Content">
                ...
                <div className="detailOpen">商品詳情<br></br>
                    <h3 className="TextPlace" id="TextPlace" dangerouslySetInnerHTML={{__html: (gBundle.content)}}></h3>
                </div>
                {/*<div className="detailOpen">商品評價</div>*/}
            </div>
    },
    componentDidMount:function (){
        gBundle.titleImage.map(function (data) {
            $('#imageList').append(`<li><img src="${data}" /></li>`)
        })
        $(".flexslider").flexslider({
            controlNav: false,
            directionNav: false, after: function (slider) {
                $('#imageIndex').html(`${slider.currentSlide + 1}/${gBundle.titleImage.length}`)
            }
        });
        $('#imageIndex').html(`1/${gBundle.titleImage.length}`)
        if($('#TextPlace').get(0)){
...
let timer=setInterval(function (){
    if(document.getElementById('ArticleContent')){
        ReactDOM.render(<ArticleContent/>,document.getElementById('ArticleContent'));
        clearInterval(timer)
    }
},100)
