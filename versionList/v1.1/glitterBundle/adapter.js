    ...
    var adapter={
        ...
        InfiniteList: createReactClass({
            getInitialState: function () {
                var it=[]
                map.item.map(function (data){
                    it.push({elementHeight:100,data:data})
                })
                return {
            ...
            handleInfiniteLoad: function () {
                ...
                adapter.getNewData(function (data) {
                    data.map(function (data){
                        adapter.reactView.state.item.push({
                            elementHeight:100,data:data
                        })
                    })
        ...
        getItems:function (){
            return adapter.reactView.state.item
        },
...
//                 for(var a=0;a<adapter.spanCount;a++){
//                     let  index=(that.props.index*adapter.spanCount)+a
//                     itemList+=(adapter.onBindViewHolder(that.props.dataList[a],index))
//                 }
//                 return <div id={`listItem` + that.props.index}
//                             dangerouslySetInnerHTML={{__html: `<div style="display: flex;width: 100%;justify-content: space-around;align-items: center;">${itemList}</div>`}}></div>
//             },
//             componentDidMount: function () {
//                 adapter.componentDidMount(this.props.index)
//                 let index = this.props.index
