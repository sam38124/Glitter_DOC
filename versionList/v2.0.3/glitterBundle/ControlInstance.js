...
var glitter = getGlitter()

var gBundle = getObject()
...
//添加多項Script
function addMtScript(urlArray, success, error) {
    var index = 0
...
//添加多項CSS
function addMtCss(urlArray, success, error) {
    var index = 0
...
function notifyDataChange(id) {
    $('#' + id).html(bindViewList[id]())
}
...
function event(fun, clid) {
    if (clid === undefined) {
        ...
        return `clickMap['${clickID}'](this,event);`
    } else {
        clickMap[clid] = fun
        return `clickMap['${clid}'](this,event);`
    }
}
 ...
 * 當內容改變及互叫Observer
 */
function addObserver(obj, callback,viewBind) {
    try {
        var map=obj.obj
        ...
        for (var a = 0; a < keys.length; a++) {
            let keyVa = keys[a];
            if (keyVa !== 'GlitterJsonStringConversionGetData' && keyVa !== 'GlitterObServerCallBack') {
